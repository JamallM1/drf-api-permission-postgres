...
class BookTests(APITestCase):
    ...
    def test_books_model(self):
        ...
        self.assertEqual(actual_title, "Percey Jackson")
        self.assertEqual(
            actual_author, "Jamall Malik"
        )
    def test_get_book_list(self):
        ...
        self.assertEqual(books[0]["title"], "Percey Jackson")
    def test_get_book_by_id(self):
        url = reverse("book_detail", args=(1,))
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        book = response.data
        self.assertEqual(book["title"], "Percey Jackson")
